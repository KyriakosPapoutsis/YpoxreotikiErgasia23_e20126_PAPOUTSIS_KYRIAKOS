...
def registerUser():
    ...
    return jsonify({'message': 'User Registration Was Successful!'}), 200
...
def searchFlights():
    ...
    if request.data:
        ...
        if originAirport and destinationAirport and date:
            ...
            return jsonify({'message': 'Not sufficient information to execute the search!'}), 400 #change http code
...
def searchFlightsAdmin():
    ...
    flights = flights_collection.find(search_query, {'_id': 1, 'date': 1, 'originAirport': 1, 'destinationAirport': 1})
    available_flights = []
    for flight in flights:
        available_flights.append({
            '_id': str(flight['_id']),
